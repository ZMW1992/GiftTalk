...
- (BOOL)insertDataWithSelectionModel:(ShouyeModel *)model
{
    ...
    // 2. 执行sql语句
    BOOL isSuccess =[_fmdb executeUpdate:sql, model.cover_image_url,model.title,model.idd];
...
- (NSArray *)selectAllData {
    ...
    while ([set next]) {
        ...
        // 将结果里面的字段值转换为模型的属性
        //model.idd = [set stringForColumn:@"selectionID"];
        model.idd = [NSNumber numberWithInt:[[set stringForColumn:@"selectionID"] intValue]];
        
        model.title = [set stringForColumn:@"selectionName"];
